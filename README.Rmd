...
- Wagentyp die Wagen-bezogenen Spektren (mit `n` bezeichnet) können `Trieb` sowohl `Wagen` Drehgestelle enthalten. 
Um Wagen-bezogenen Spektren aus ähnlichen Drehgestelle zu vergleichen, werden die Intervalle jede Vorbeifahrt in `Wagen` , `Trieb/Wagen` kategorisiert. Die Kategorisierung erfolgt mithilfe von Achsabstand.
...
- [**Diskussion über Spektren und Gesamtpegel von Vorbeifahrten**](`r if(!word){paste( html_render, "Vorbeifahrten.html", sep = "")} `) um einen Gefühl mit die gemessene/ausgewertete Werte zu bekommen.
    
- [**Abschätzen der Wirkung von Schienendämpfer**](`r paste( html_render, "SD_wirkung.html", sep = "") `). Die Pegeldifferenz zwischen Referenzabschnitt und Dämpferabschnitt  wird berechnet und diskutiert.

...
```
Die Dämpferwirkung lässt sich durch die Differenzen der Spektren bzw. Levels zwischen  Referenzabschnitt und Dämpferabschnitt abschätzen. 
...
ggplot(data = diffDf%>%
  ...
  scale_y_continuous(breaks=-5:5)+
  ylab('Level dBA')+
facet_grid( .~ interval.kat )+ theme_bw()+ theme(legend.position="bottom")
```
...
if(word){
  ...
  datatable(table, filter = 'top', rownames = FALSE, options = list(pageLength = nrow(table), dom = 'tip',paging = FALSE))
}
```

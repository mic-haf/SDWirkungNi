---
title: "Gemessene Schienenrauheit in Niederwangen und  Schalldruckspektrum Korrekturen"
author: "ESR"
...
opts_chunk$set(message = FALSE, 
                      ...
                      #dev='CairoPDF'
                      #fig.ext = 'png',
                      #engine.opts = list(convert.opts = '-density 300')
                      )
...
#
df_R = R_fromdb(cursor,conn, Rvar,OBsetupVar)
######################################################
# uniform wl da ACCON and MBBM have different rounding
wlMBBM = sort(as.numeric(levels(factor(df_R$wl[df_R$Author=='MBBM']))))

df_R$wl = sapply(df_R$wl, function(x){
  y=abs(x-wlMBBM)
...
Messungen = df_R%>%group_by(Author,kampagne,messdatum)%>%summarize(Messdatum = strftime(messdatum[1], '%B-%Y') )
...
```{r out.width=1000}
df_R = df_R %>%  mutate(months = as.numeric(difftime( messdatum,schleifVorgang, units = "days"))/30)
...
```{r,out.width=1000}
df_R_filt= droplevels(df_R %>% filter( Author == 'ACCON' | kampagne == 3|kampagne==4))
av.a_df_R = df_R_filt %>%
  group_by(sec,wl)%>%
  summarize(fall='a', avR = 10*log10(sum(10^(R/10))/length(R)) )

RauheitsPlot()+
  geom_line(data=df_R_filt,
            aes(x=wl,y=R, group = r_id, colour = months,linetype = Author ),
            size = 1.5,alpha = 0.8)+
  geom_line(data = av.a_df_R,
          aes(x=wl,y=avR),
          size = 1.5,alpha = 0.8,colour = 'red')+
  facet_wrap(~sec,ncol = 2)

# fall b
df_R_filt= df_R %>% filter( kampagne == 3|kampagne==4)
av.b_df_R = df_R_filt %>%
  group_by(sec,wl)%>%
  summarize(fall='b', avR = 10*log10(sum(10^(R/10))/length(R)) )
# fall c
df_R_filt= df_R %>% filter( kampagne==4)
av.c_df_R = df_R_filt %>%
  group_by(sec,wl)%>%
  summarize(fall='c', avR = 10*log10(sum(10^(R/10))/length(R)) )
# rbind
av_df_R=rbind(av.a_df_R,av.b_df_R,av.c_df_R)
rm(av.a_df_R,av.b_df_R,av.c_df_R,df_R_filt)
```
